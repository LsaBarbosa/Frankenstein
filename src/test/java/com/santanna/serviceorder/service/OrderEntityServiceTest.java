...
import com.santanna.serviceorder.refatoracao.domain.service.OrderService;
import com.santanna.serviceorder.refatoracao.infrastructure.repository.OrderRepository;
...
class OrderEntityServiceTest {
    ...
    void shouldThrowBadRequestException_WhenOrderAlreadyExists() {
        when(orderRepository.findByOrderNumber(orderRequestDto.getOrderNumber())).thenReturn(Optional.of(new OrderEntity()));
    ...
    void shouldThrowBadRequestException_WhenValidationFails() {
        when(orderRepository.findByOrderNumber(orderRequestDto.getOrderNumber())).thenReturn(Optional.empty());
        when(orderRepository.save(any(OrderEntity.class))).thenThrow(new ConstraintViolationException("Validation Error", null));
    ...
    void shouldThrowBadRequestException_OnDataIntegrityViolation() {
        when(orderRepository.findByOrderNumber(orderRequestDto.getOrderNumber())).thenReturn(Optional.empty());
        when(orderRepository.save(any(OrderEntity.class))).thenThrow(new DataIntegrityViolationException("Constraint Error"));
    ...
    void shouldThrowInternalServerErrorException_OnUnexpectedError() {
        when(orderRepository.findByOrderNumber(orderRequestDto.getOrderNumber())).thenReturn(Optional.empty());
        when(orderRepository.save(any(OrderEntity.class))).thenThrow(new RuntimeException("Unexpected error"));
    ...
    void shouldThrowInternalServerErrorException_WhenUnexpectedErrorUpdatingOrder() {
        ...
        when(orderRepository.save(any(OrderEntity.class))).thenThrow(new RuntimeException("Unexpected save error"));
    ...
    void shouldGetAllOrdersWithPaginationSuccessfully() {
        ...
        when(orderRepository.findAll(pageable)).thenReturn(new PageImpl<>(List.of(new OrderEntity(), new OrderEntity())));
